...
var (
	...
	ERR_API_CLIENT_ERROR_MESSAGE             = errors.TN(INLET_HTTP_API_CLIENT_ERR_NS, 4, "remote server response error, api: {{.api}}, url: {{.url}} error namespace: {{.err_namespace}}, code: {{.code}}, message: {{.message}}, error id: {{.err_id}}")
)
